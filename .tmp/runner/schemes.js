...
const getIncomeRepresentative = (incomeRange) => {
...
const getIncomeRangeLabel = (incomeRange) => INCOME_RANGE_BOUNDS[incomeRange].label;
...
function calculateEligibility(profile, scheme) {
    ...
    if (elig.maxIncome !== undefined && incomeBounds.min > elig.maxIncome) {
        return disqualify(`Income limit is Rs.${elig.maxIncome.toLocaleString("en-IN")}/year (selected range: ${incomeRangeLabel})`);
    }
...
function getMatchedSchemes(profile) {
    ...
    const isPreferredStateScheme = (match) => {
        if (match.scheme.level !== "state")
            return false;
...
function calculateBenefitBreakdown(matches) {
    ...
    const sumExclusiveGroupMax = (pool) => mutuallyExclusiveCashGroups.reduce((sum, group) => {
        const groupMax = pool
            .filter((match) => group.includes(match.scheme.id) &&
    ...
    const directSupportTotal = Math.round(directAdditiveCashAndSubsidy + directExclusiveAlternatives);
    ...
    const conditionalLoan = conditionalMatches
        ...
        .reduce((max, match) => Math.max(max, weightedValue(match)), 0);
    const conditionalSupportTotal = Math.round(conditionalAdditiveCashAndSubsidy +
