export interface Scheme {
  id: string
  name: string
  category: "pension" | "housing" | "agriculture" | "education" | "business" | "health" | "women" | "employment" | "social-security"
  level: "central" | "state"
  state?: string
  description: string
  benefits: string
  annualValue: number // Estimated annual value in INR
  eligibility: {
    minAge?: number
    maxAge?: number
...
export function calculateEligibility(profile: UserProfile, scheme: Scheme): SchemeMatch {
  ...
  const occupationRelatedMap: Record<string, string[]> = {
